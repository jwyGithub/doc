...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (role === 'admin' && currentUser.role !== 'superadmin') {
            return Response.json({ error: 'Only super admin can set admin role' }, { status: 403 });
        }
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (currentUser.id === id) {
            return Response.json({ error: 'Cannot delete yourself' }, { status: 403 });
        }
        ...
        if (targetUser.role === 'admin' && currentUser.role !== 'superadmin') {
            return Response.json({ error: 'Only super admin can delete admin' }, { status: 403 });
        }
